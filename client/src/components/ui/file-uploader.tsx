...
const FileUploader = ({
     ...
     const [uploadedFiles, setUploadedFiles] = useState<readonly FileWithPath[]>([]); // Ensure it's initialized as an empty array
     ...
     return (
          <div className={cn('relative mx-4 min-h-48 min-w-48 cursor-pointer', className)}>
          <div
              {...getRootProps()}
          ...
          <div className="space-y-2">
              {!hasError && uploadedFiles.map((file, idx) => (
                  <UploadedFileItem
                      key={idx}
                      file={file}
