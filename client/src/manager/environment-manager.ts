...
export class EnvironmentManager {
    ...
    public static async initialize(): Promise<void> {
        await this.storeEncryptedEnvVariable(
        ...
        await this.storeEncryptedEnvVariable(
            'VITE_CLIENT_SERVER_URL',
            import.meta.env.VITE_CLIENT_SERVER_URL
        )
        await this.storeEncryptedEnvVariable(
            'VITE_CLIENT_DOCUMENT_URL',
            import.meta.env.VITE_CLIENT_DOCUMENT_URL
        )
        await this.storeEncryptedEnvVariable(
            'VITE_CLIENT_GOOGLE_API_KEY',
            import.meta.env.VITE_CLIENT_GOOGLE_API_KEY
        )
    }

    // Store encrypted environment variable (static)
    private static async storeEncryptedEnvVariable(
        key: string,
        value: string
    ): Promise<void> {
        if (!value)
    ...
    public static async getDecryptedEnvVariable(key: string): Promise<string> {
        ...
        if (!encryptedValue)
            throw new Error(
                `Encrypted environment variable ${key} is not found.`
            )
