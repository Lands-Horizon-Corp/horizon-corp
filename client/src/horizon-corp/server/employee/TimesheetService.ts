...
export default class TimesheetService {
   ...
   */
  public static async timeIn(timeInData: TimeInRequest): Promise<TimesheetResource> {
   ...
   */
  public static async timeOut(timeOutData: TimeOutRequest): Promise<TimesheetResource> {
    const endpoint = `${TimesheetService.BASE_ENDPOINT}/time-out`;
    const response = await UseServer.post<TimeOutRequest, TimesheetResource>(
      endpoint,
      timeOutData
    );
    return response.data;
