...
func NewLoggerProvider(cfg *config.AppConfig) (*LoggerService, error) {
	...
	case "production":
		...
		if err != nil {
			return nil, eris.Wrap(err, "failed to create production logger")
		}
	...
	default:
		...
		if err != nil {
			return nil, eris.Wrap(err, "failed to create development logger")
		}
	}
...
func (ls *LoggerService) Sync() error {
	err := ls.Client.Sync()
	if err != nil {
		return eris.Wrap(err, "failed to sync logger")
	}
	return nil
}
