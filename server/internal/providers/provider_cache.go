...
func NewCacheProvider(
	...
	if cfg == nil {
		return nil, eris.New("configuration is nil")
	}

	if cfg.CacheURL == "" {
		logger.Error("Cache URL is missing from configuration.")
		return nil, eris.New("cache URL configuration missing")
	}
	...
	logger.Error("Could not connect to Redis after multiple attempts",
		zap.Int("maxRetries", maxRetries),
		zap.Error(lastErr))
	return nil, eris.Wrapf(lastErr, "could not connect to Redis after %d attempts", maxRetries)
}
...
func (cs *CacheService) Delete(key string) error {
	...
	if err != nil {
		cs.logger.Error("Failed to delete key from cache",
			zap.String("key", key),
			zap.Error(err))
		return eris.Wrap(err, "failed to delete key from cache")
	}
	return nil
}
...
func (cs *CacheService) Exists(key string) (bool, error) {
	...
	if err != nil {
		cs.logger.Error("Failed to check if key exists in cache",
			zap.String("key", key),
			zap.Error(err))
		return false, eris.Wrap(err, "failed to check if key exists in cache")
	}
	return val > 0, nil
}
...
func (cs *CacheService) Set(key string, value interface{}, expiration time.Duration) error {
	...
	if err != nil {
		cs.logger.Error("Failed to set key in cache",
			zap.String("key", key),
			zap.Error(err))
		return eris.Wrap(err, "failed to set key in cache")
	}
	return nil
}
...
func (cs *CacheService) Get(key string) (string, error) {
	...
	if err == redis.Nil {
		cs.logger.Debug("Cache miss",
			zap.String("key", key))
		return "", eris.New(cacheMissErrString)
	} else if err != nil {
		cs.logger.Error("Failed to get key from cache",
			zap.String("key", key),
			zap.Error(err))
		return "", eris.Wrap(err, "failed to get key from cache")
	}
	return val, nil
}
...
func (cs *CacheService) Increment(key string) (int64, error) {
	...
	if err != nil {
		cs.logger.Error("Failed to increment key in cache",
			zap.String("key", key),
			zap.Error(err))
		return 0, eris.Wrap(err, "failed to increment key in cache")
	}
	return val, nil
}
...
func (cs *CacheService) Decrement(key string) (int64, error) {
	...
	if err != nil {
		cs.logger.Error("Failed to decrement key in cache",
			zap.String("key", key),
			zap.Error(err))
		return 0, eris.Wrap(err, "failed to decrement key in cache")
	}
	return val, nil
}
...
func (cs *CacheService) Ping() error {
	...
	if err != nil {
		return eris.Wrap(err, "failed to ping Redis")
	}
	return nil
}
